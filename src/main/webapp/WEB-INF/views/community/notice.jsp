	...
	<div class="container">
		...
		<div id="searchKeyword" style="height: 60px; margin: 0px auto; text-align: center;">
			<select name="searchType" id="searchType">
				<option value="title" <c:if test="${condition eq 'notice_title'}"> selected</c:if>>제 목</option>
			</select> 
		...
		<form action="inputNotice" method="GET">
			<input type="hidden" name="keyword" class="keyword" value='<c:out value="${pageDto.keyword}" />'>
			...
			<input type="hidden" id="user_email" name="user_email" value='<c:out value="${user.user_email}" />'>
				...
				<tbody>
					<c:forEach items="${list }" var="dto">
						<tr>
							...
							<%--<td><a href="/community/noticeDetail?notice_seq=${dto.user_no}">${dto.notice_title }</a></td>--%>
							<td><a href="/community/noticeDetail?notice_seq=${dto.notice_seq}">${dto.notice_title }</a></td>
			...
			<div class="m-paging">
				<ul>
					<c:if test="true">
						<li>
							<a href='<c:url value="/community/noticeList?page=${dto.startPage-1 }&keyword=${dto.keyword}"/>' id="pre" class='oiBtn prev'>◀</a>
						</li>
					</c:if>
					<c:forEach begin="${dto.startPage}" end="${dto.endPage}" var="page">
						<li>
							<a href='<c:url value="/community/noticeList?page=${page}&amount=${cri.amount}&keyword=${dto.keyword}"/>' class='num <c:if test="${dto.cri.page eq page}"> active </c:if>'>${page}</a>
						</li>
					</c:forEach>
					<c:if test="${dto.next && dto.endPage>0}">
						<li>
							<a href='<c:url value="/community/notice?page=${dto.endPage+1 }&keyword=${dto.keyword}"/>' id="next" class='oiBtn next'>▶</a>
						</li>
					</c:if>
				</ul>
			</div>
