...
<html lang="en">
    <head>
        ...
        <title>Admin_Application_List</title>
    ...
    <body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav">
                ...
                <nav class="sb-sidenav bg-info p-2 text-dark bg-opacity-25" id="sidenavAccordion">
                    <div class="sb-sidenav-menu">
                        <div class="nav" style="text-align: center;">
                            ...
                            <a class="nav-link application" href="adminAppList.do" style="color: #9966FF;"><div class="sb-nav-link-icon"><i class="fa fa-shoe-prints" aria-hidden="true"></i></div><span>Application</span></a>
                        </div>
                    </div>
                </nav>
            </div>
            <div id="layoutSidenav_content">
                <main>
                	<br>
                    <div class="container-fluid px-4 appList">
                        <h1 class="title_tab">강의 신청 관리</h1>
                        <br><br>
                        <div class="card mb-4">
                            <div class="card-header"><i class="fas fa-table me-1"></i>강의 신청 목록</div>
                            <div class="card-body">
							...
							<div id="searchKeyword">
									...
									<select name="searchType" id="searchType">
										<option value="title" <c:if test="${condition eq 'title'}"> selected</c:if>>이 름</option>
									</select> 
									
									<input type="text" name="keyword" id="keyword" value="${dto.keyword}" size="35" placeholder="검색할 이름을 입력하세요"/> 
	                                    	...
	                                    	<c:choose>
			                                    ...
			                                    <c:otherwise>
													<c:forEach items="${list}" var="dto">
						                            	<tr>
						                                	<td><a href="javascript:;" class="appDetail" id="${dto.app_no}">${dto.rnum}</a></td>
						                                    <td><a href="javascript:;" class="appDetail" id="${dto.c_no}">${dto.course.c_name}</a></td>
						                                    <td>${dto.user_name}</td>
						                                    <td>${dto.app_date}</td>
		                            	...
		                            	<div class="coursePaging">
											<div class="m-paging">
												<ul>
													<c:if test="true">
														<li>
															<a href='<c:url value="adminAppList.do?page=${dto.startPage-1 }&keyword=${dto.keyword}"/>' id="pre" class='oiBtn prev'>◀</a>
														</li>
													</c:if>
													
													<c:forEach begin="${dto.startPage}" end="${dto.endPage}" var="page">
														<li>
															<a href='<c:url value="adminAppList.do?page=${page}&amount=${cri.amount}&keyword=${dto.keyword}"/>' class='num <c:if test="${dto.cri.page eq page}"> active </c:if>'>${page}</a>
														</li>
													</c:forEach>
													
													<c:if test="${dto.next && dto.endPage>0}">
														<li>
															<a href='<c:url value="adminAppList.do?page=${dto.endPage+1 }&keyword=${dto.keyword}"/>' id="next" class='oiBtn next'>▶</a>
														</li>
													</c:if>
												</ul>
											</div>
        ...
        $(document).ready(function(){
			$("#pre, .num, #next, #searchBtn").on("click", function(){
				...
				form.attr("action", "<c:url value='/adminAppList.do'/>");
			...
			$(".appDetail").on("click", function(){
				...
				let app_no = $(this).attr('id');
				let c_no = $(this).attr('id');
				
				let form= $("<form></form>");
				form.attr("name", "appDetail");
				form.attr("method", "get");
				form.attr("action", "<c:url value='adminAppDetail.do'/>");
				
				form.append($("<input />", {type: "hidden", name: "keyword", value: keyword}));
				form.append($("<input />", {type: "hidden", name: "searchType", value: searchType}));
				form.append($("<input />", {type: "hidden", name: "page", value: page}));
				form.append($("<input />", {type: "hidden", name: "amount", value: amount}));
				form.append($("<input />", {type: "hidden", name: "app_no", value: app_no}));
